...
class StaticContractTest(unittest.TestCase):
    ...
    def test_static_asset_version_is_consistent(self):
        for name in ("index.html", "records.html", "record.html"):
            ...
            self.assertIn("styles.css?v=20260703-28", html)
            self.assertIn("ui-utils.js?v=20260703-28", html)
            self.assertIn("app-data.js?v=20260703-28", html)
            self.assertIn("EVENT INFO ＆ CUSTOMER NOTE", html)
            self.assertNotIn("IVENT INFO", html)

        service_worker = (ROOT / "sw.js").read_text(encoding="utf-8")
        self.assertIn("const VERSION = '20260703-28';", service_worker)
    ...
    def test_core_ui_contracts(self):
        ...
        self.assertIn(".week-date strong { width: 96px;", styles)
