...
class StaticContractTest(unittest.TestCase):
    ...
    def test_static_asset_version_is_consistent(self):
        for name in ("index.html", "records.html", "record.html"):
            ...
            self.assertIn("styles.css?v=20260718-02", html)
            self.assertIn("ui-utils.js?v=20260718-02", html)
            self.assertIn("app-data.js?v=20260718-02", html)
            self.assertIn("EVENT INFO", html)
            self.assertIn('<div class="brand-title"><h1>イベント情報</h1><p class="eyebrow">EVENT INFO</p></div>', html)
            self.assertIn('<strong>イベント情報</strong><span>EVENT INFO</span>', html)
            self.assertNotIn("CUSTOMER NOTE", html)
            self.assertNotIn("イベント情報・集客記録", html)
            self.assertNotIn("IVENT INFO", html)

        service_worker = (ROOT / "sw.js").read_text(encoding="utf-8")
        self.assertIn("const VERSION = '20260718-02';", service_worker)
