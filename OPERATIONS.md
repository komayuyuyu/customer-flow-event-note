...
1. 調査した候補を `data/candidates.json` に書く。
   ...
   - 三井アウトレットパーク マリンピア神戸公式サイトの「営業時間・休館日」（`https://mitsui-shopping-park.com/mop/kobe/hour/`）と「お知らせ」（`https://mitsui-shopping-park.com/mop/kobe/information/`）を、毎朝・週次チェックの固定巡回先にする。特別営業時間、開店前倒し、閉店延長、臨時休館と、その更新・取消を確認する。
   - 営業時間変更は `data/store-events.json` に公式URL付きで保存し、`displayOrder: 10` を付ける。画面では祝日・大型連休を最上段に保ち、その直下かつ通常イベントの先頭に表示する。
