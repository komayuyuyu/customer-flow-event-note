...
1. 調査した候補を `data/candidates.json` に書く。
   ...
   - 三井アウトレットパーク マリンピア神戸公式サイトの「営業時間・休館日」（`https://mitsui-shopping-park.com/mop/kobe/hour/`）と「お知らせ」（`https://mitsui-shopping-park.com/mop/kobe/information/`）を、毎朝・週次チェックの固定巡回先にする。公式ページに記載された通常のショップ営業時間と対象日の営業時間を比較し、開店・閉店時刻または休館日に変更がある日だけ掲載する。通常どおりの日は掲載しない。レストラン、フードコート、フードマルシェなどショップ以外の営業時間も掲載しない。
   - ショップの営業時間変更は `data/store-events.json` に公式URL付きで保存し、`displayOrder: 10` を付ける。画面は `大型連休 → 祝日 → 営業時間変更 → その他イベント` の順に表示する。
