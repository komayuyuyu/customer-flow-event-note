...
1. 調査した候補を `data/candidates.json` に書く。
   ...
   - タイトルを言い換えただけの説明や、見れば分かる定型説明文は表示しない。「視聴準備・早めの帰宅」は時間帯ごと表示しない。「リアルタイム視聴」はラベルと時刻を残し、「生中継・ライブ配信をその時間に見る人が増える可能性」という理由文だけを表示しない。その他の予測時間帯の説明文は、交通規制や特殊な混雑など追加情報がある時だけ表示する。
